      ...
      <main className='relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-12'>
        ...
        <section className='relative z-10 w-full max-w-3xl rounded-[30px] border border-white/60 bg-white/80 p-8 text-center shadow-[0_20px_80px_-28px_rgba(86,81,229,0.55)] backdrop-blur-md sm:p-12'>
          ...
          <div className='mx-auto mt-7 flex max-w-xl flex-wrap items-center justify-center gap-3'>
            {quickLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <a className='rounded-full border border-indigo-100 bg-indigo-50/80 px-4 py-2 text-sm font-semibold text-[#5651e5] transition hover:border-indigo-200 hover:bg-indigo-100'>
                  {link.label}
                </a>
              </Link>
            ))}
          ...
          <div className='mt-8 grid gap-4 sm:grid-cols-2'>
            <Link href='/'>
              <a className='inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-[#5651e5] to-[#709dff] px-6 py-3 font-semibold text-white shadow-lg shadow-indigo-300/60 transition hover:scale-[1.02]'>
                Voltar para a home
              </a>
            </Link>
